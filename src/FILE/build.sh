...
{
CHECK_LISTS[0]="${C2X2K_PREFIX_PATH}/lib${C2X2K_TARGET_BITWIDE}/libmagic.la"
CHECK_LISTS[1]="${C2X2K_PREFIX_PATH}/lib${C2X2K_TARGET_BITWIDE}/libmagic.so"
CHECK_LISTS[2]="${C2X2K_PREFIX_PATH}/lib/libmagic.la"
CHECK_LISTS[3]="${C2X2K_PREFIX_PATH}/lib/libmagic.so"
}
else
{
...
./configure --host=${C2X2K_TARGET_MACHINE} \
    ...
    CFLAGS="-fPIC -I${C2X2K_PREFIX_PATH}/include" \
    LDFLAGS="-L${C2X2K_PREFIX_PATH}/lib${C2X2K_TARGET_BITWIDE}/ -L${C2X2K_PREFIX_PATH}/lib" \
    >>${C2X2K_BUILD_LOG_FILE} 2>&1 
exit_if_error $? "Failed to configure ${PROJECT_NAME}." $?

#更新PATH和LD_LIBRARY_PATH环境变量,用于后续編译时找到本地file工具.
export PATH=${C2X2K_PREFIX_PATH}/../${C2X2K_NATIVE_RELEASE_NAME}/bin:$PATH
export LD_LIBRARY_PATH=${C2X2K_PREFIX_PATH}/../${C2X2K_NATIVE_RELEASE_NAME}/lib${C2X2K_NATIVE_BITWIDE}:${C2X2K_PREFIX_PATH}/../${C2X2K_NATIVE_RELEASE_NAME}/lib:${LD_LIBRARY_PATH}
