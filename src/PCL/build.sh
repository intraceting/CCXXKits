...
#
if [ $(check_keyword ${BUILD_FLAGS} "rebuild-pcl") -eq 0 ];then
{
CHECK_LISTS[0]="${C2X2K_TARGET_PREFIX}/lib${C2X2K_TARGET_BITWIDE}/libpcl_common.a"
CHECK_LISTS[1]="${C2X2K_TARGET_PREFIX}/lib${C2X2K_TARGET_BITWIDE}/libpcl_common.so"
CHECK_LISTS[2]="${C2X2K_TARGET_PREFIX}/lib/libpcl_common.a"
CHECK_LISTS[3]="${C2X2K_TARGET_PREFIX}/lib/libpcl_common.so"
}
else
{
CHECK_LISTS[0]="/tmp/rebuild-pcl"
}
fi
...
#
SRC_FILE=${SHELLDIR}/pcl-1.12.1.tar.xz
#
...
#    -DCMAKE_C_FLAGS="-fPIC -DBOOST_BIND_GLOBAL_PLACEHOLDERS -DEIGEN_DONT_ALIGN_STATICALLY -Wno-narrowing  -D_GLIBCXX_USE_C99_MATH" \
#    -DCMAKE_CXX_FLAGS="-fPIC -DBOOST_BIND_GLOBAL_PLACEHOLDERS -DEIGEN_DONT_ALIGN_STATICALLY -Wno-narrowing  -D_GLIBCXX_USE_C99_MATH" \
...
${C2X2K_NATIVE_CMAKE_BIN} ${SRC_PATH} \
    ...
    -DCMAKE_C_FLAGS="-fPIC -DBOOST_BIND_GLOBAL_PLACEHOLDERS -DEIGEN_DONT_ALIGN_STATICALLY -Wno-narrowing" \
    -DCMAKE_CXX_FLAGS="-fPIC -DBOOST_BIND_GLOBAL_PLACEHOLDERS -DEIGEN_DONT_ALIGN_STATICALLY -Wno-narrowing" \
    -DCMAKE_EXE_LINKER_FLAGS="-Wl,-rpath-link=${C2X2K_TARGET_PREFIX}/lib${C2X2K_TARGET_BITWIDE} -Wl,-rpath-link=${C2X2K_TARGET_PREFIX}/lib" \
    -DCMAKE_SHARED_LINKER_FLAGS="-Wl,-rpath-link=${C2X2K_TARGET_PREFIX}/lib${C2X2K_TARGET_BITWIDE} -Wl,-rpath-link=${C2X2K_TARGET_PREFIX}/lib" \
...
#编译.不能开太多并行編译，会占用大量内存.
make -j4 >>${C2X2K_BUILD_LOG_FILE} 2>&1
