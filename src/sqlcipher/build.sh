...
#
if [ $(check_keyword ${BUILD_FLAGS} "rebuild-sqlite") -eq 0 ];then
{
CHECK_LISTS[0]="${C2X2K_TARGET_PREFIX}/lib${C2X2K_TARGET_BITWIDE}/libsqlite3.a"
CHECK_LISTS[1]="${C2X2K_TARGET_PREFIX}/lib${C2X2K_TARGET_BITWIDE}/libsqlite3.so"
CHECK_LISTS[2]="${C2X2K_TARGET_PREFIX}/lib/libsqlite3.a"
CHECK_LISTS[3]="${C2X2K_TARGET_PREFIX}/lib/libsqlite3.so"
}
else
{
CHECK_LISTS[0]="/tmp/rebuild-sqlite"
}
fi
...
#
SRC_FILE=${SHELLDIR}/sqlcipher-4.12.0.tar.gz
#
...
./configure \
    ...
    --fts3 \
    --all \
    --with-icu-ldflags="-licui18n -licuuc -licudata -L${C2X2K_TARGET_PREFIX}/lib${C2X2K_TARGET_BITWIDE} -L${C2X2K_TARGET_PREFIX}/lib -Wl,-rpath-link=${C2X2K_TARGET_PREFIX}/lib${C2X2K_TARGET_BITWIDE} -Wl,-rpath-link=${C2X2K_TARGET_PREFIX}/lib" \
    --with-icu-cflags="-I${C2X2K_TARGET_PREFIX}/include/" \
    CC=${C2X2K_TARGET_COMPILER_C} \
    CXX=${C2X2K_TARGET_COMPILER_CXX} \
    CFLAGS="-std=c99 -fPIC -D_GNU_SOURCE -I${C2X2K_TARGET_PREFIX}/include/" \
    CXXFLAGS="-std=c++11 -fPIC -D_GNU_SOURCE -I${C2X2K_TARGET_PREFIX}/include/" \
